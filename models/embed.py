...
#     def __init__(self, d_model, code_vocab_size):
#         super().__init__()
#         self.code_emb = nn.Embedding(code_vocab_size, d_model,padding_idx=0)

#     def forward(self, code_idx): 
        
#         code_emb=self.code_emb(code_idx)
            
#         return code_emb

class MultiCodeEmbedding(nn.Module):
    def __init__(self, d_model: int, code_vocab_size: int):
        super().__init__()
        self.code_emb = nn.Embedding(code_vocab_size, d_model, padding_idx=0)

    def forward(self, code_idx: torch.LongTensor) -> torch.Tensor:
...
class CodeDataembedding(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', code_vocab_size=311, dropout=0.1):
        ...
        self.position_embedding = PositionalEmbedding(d_model=d_model)
        self.temporal_embedding = TemporalEmbedding(d_model=d_model, embed_type=embed_type, freq=freq) if embed_type!='timeF' else TimeFeatureEmbedding(d_model=d_model,                                                        embed_type=embed_type, freq=freq)
        self.code_embedding= MultiCodeEmbedding(d_model=d_model, code_vocab_size=code_vocab_size)
